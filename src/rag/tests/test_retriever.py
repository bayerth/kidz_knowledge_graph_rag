...
class TestInformationRetrieverWithGraph(unittest.TestCase):
    ...
    @patch("ldrag.gpt_request", return_value='["Person"]')
    @patch("ldrag.gpt_request_with_history", return_value=('["Person_instance"]', []))
    @patch("ldrag.execute_query", return_value=[MagicMock(get_node_id=lambda: "Person_instance")])
    @patch("ldrag.create_rag_instance_graph", return_value="static/graph/rag_test.html")
